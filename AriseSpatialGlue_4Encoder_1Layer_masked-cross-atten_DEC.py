...
"""
================================================================================
  🧬 AriseSpatialGlue_4Encoder_1Layer_masked-cross-atten_DEC.py
  ...
  3. ⚡ Spatial Graph-Masked Cross-Attn (V11) (Refines RNA with Aux features)
  ...
  NOTE: This is Base Model + DEC + Masked Cross-Attention.
        No Gram Alignment, No Sinkhorn OT, No Kendall & Gal Uncertainty Balancing.
================================================================================
"""
...
class DualGCNAll1Layer(nn.Module):
    ...
    def forward(self, x_RNA, x_ADT,
        ...
        return x_sim, x_dist, aux_s, aux_d, fused_rna, fused_aux
...
class Dual4Encoder1LayerDEC(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, out_channels, q, num_clusters,
                 beta=25.0, gamma=10.0, delta=1.0, kl_weight=1.0, dropout=0.0,
        ...
        self.cross_attn = GraphMaskedCrossAttention(d_model=out_channels, n_heads=4, dropout=0.1)
    ...
    def forward(self, data: Dual4GraphDataDEC, compute_q: bool = False):
        ...
        """
        x_sim, x_dist, aux_s, aux_d, fused_rna, fused_aux = self.gcn(
        ...
        fused_pro = self.fusion_layer2(torch.cat([z_rna_refined, fused_aux], dim=1))
...
def run_experiment(
    ...
    output_dir: str = "results_masked_attn_dec",
    data_dir: str = "data"
):
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default='results_masked_attn_dec', help="Directory to save CSV results and plots")
