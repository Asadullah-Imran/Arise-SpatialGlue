...
class ASTRA_v1_DEC_Gated(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, q_dim, num_clusters,
                 delta=1.0, lambda_spatial=0.15, dropout=0.0):
    ...
    def compute_losses(self, data, sim_z, dist_z, fused_z, fused_pro, pro, kl_loss=None, stage=1):
        ...
        if stage == 2 and kl_loss is not None:
            ...
            total_loss = total_loss + 0.5 * p2 * kl_loss + 0.5 * self.log_vars[2]
...
def train_model_dec(
    ...
    for epoch in range(pretrain_epochs):
        ...
        if (epoch + 1) % 25 == 0 or epoch == 0 or (epoch + 1) == pretrain_epochs:
            print(f"Pretrain Ep {epoch+1:3d}/{pretrain_epochs} [p0={torch.exp(-model.log_vars[0]):.2f} p1={torch.exp(-model.log_vars[1]):.2f}] | Tot: {loss.item():.4f} | Rec: {l_rec.item():.4f} | Spat: {l_sp.item():.4f} | KL: 0.0000 | Sim: {l_sim.item():.4f} | Dist: {l_dist.item():.4f} | Aux: {l_aux.item():.4f} | Sil: {sil:.4f} (Best: {best_sil:.4f} @ Ep {best_epoch}) | ARI: {ari:.4f}")
    ...
    for epoch in range(finetune_epochs):
        ...
        if (epoch + 1) % 25 == 0 or epoch == 0 or (epoch + 1) == finetune_epochs or sil > best_sil_stage1:
            print(f"DEC Ep {epoch+1:3d}/{finetune_epochs} (Total {curr_epoch:3d}) [p0={torch.exp(-model.log_vars[0]):.2f} p1={torch.exp(-model.log_vars[1]):.2f} p2={torch.exp(-model.log_vars[2]):.2f}] | Tot: {loss.item():.4f} | Rec: {l_rec.item():.4f} | Spat: {l_sp.item():.4f} | KL: {kl_val:.4f} | Sim: {l_sim.item():.4f} | Dist: {l_dist.item():.4f} | Aux: {l_aux.item():.4f} | Sil: {sil:.4f} (Peak: {best_sil:.4f} [{best_stage}] @ Ep {best_epoch}) | ARI: {ari:.4f}")
...
def plot_all_visualizations(
    ...
    if spatial_coords is not None:
        ...
        axes[1].set_title(f'ASTRA-Redo Kendall-Gal Domains (ARI: {ari:.4f})', fontsize=12, fontweight='bold')
...
def run_experiment(datasets="all", seeds=None, pretrain_epochs=250, finetune_epochs=150, lr=1e-3, 
                   lambda_spatial=0.15, output_dir="results/astra_redo_kendal_gal_Results", 
                   visualize=True):
    ...
    for dataset_name in dataset_names:
        ...
        for seed in seeds:
            ...
            model = ASTRA_v1_DEC_Gated(
                ...
                delta=1.0, lambda_spatial=lambda_spatial
            ).to(device)
        ...
        df_ds.to_csv(os.path.join(output_dir, f"ASTRA_Redo_Kendall_Gal_{dataset_name}_results.csv"), index=False)
        
    df_all = pd.DataFrame(all_results)
    df_all.to_csv(os.path.join(output_dir, "ASTRA_Redo_Kendall_Gal_all_results.csv"), index=False)
