...
class MultimodalGraphData(Data):
    def __init__(self, x_RNA, x_ADT, sim_edge_index, sim_edge_weight,
                 ...
                 spatial_adj=None, motif_matrix=None):
        super().__init__()
        self.x_RNA = x_RNA
        self.x_ADT = x_ADT
        ...
        self.comm_edge_index = common_edge_index
        self.comm_edge_weight = common_edge_weight
        self.spatial_adj = spatial_adj
        self.motif_matrix = motif_matrix


def normalize_adj_tensor(adj_sparse: torch.Tensor) -> torch.Tensor:
    num_nodes = adj_sparse.shape[0]
    indices = adj_sparse.indices()
    values = adj_sparse.values()

    deg = torch.zeros(num_nodes, device=adj_sparse.device)
...
def build_multimodal_graphs(RNA_expression: np.ndarray,
                            ADT_expression: np.ndarray,
                            cell_positions: np.ndarray,
                            device: str = 'cpu',
                            num_neighbors: int = 15) -> MultimodalGraphData:
    ...
    adj_sim = np.zeros_like(similarity_matrix, dtype=int)
    ...
    sim_edge_weight = torch.tensor(similarity_matrix[adj_sim > 0], dtype=torch.float).to(device)
    ...
    # Normalized Spatial Adjacency
    spatial_adj_sparse = torch.sparse_coo_tensor(dist_edge_index, torch.ones_like(dist_edge_weight), (num_nodes, num_nodes)).coalesce()
    spatial_adj_norm = normalize_adj_tensor(spatial_adj_sparse)

    x_RNA_tensor = torch.tensor(RNA_expression, dtype=torch.float).to(device)
    x_ADT_tensor = torch.tensor(ADT_expression, dtype=torch.float).to(device)

    return MultimodalGraphData(
        x_RNA=x_RNA_tensor,
        x_ADT=x_ADT_tensor,
...
class ARISEBaselineModel(nn.Module):
    def __init__(self, in_rna_dim: int, in_aux_dim: int, hidden_dim: int = 512, out_dim: int = 64,
                 ...
                 l1_lambda: float = 1e-4, l2_lambda: float = 1e-3, **kwargs):
        super(ARISEBaselineModel, self).__init__()
        self.beta, self.gamma, self.delta = beta, gamma, delta
        self.l1_lambda, self.l2_lambda, self.dropout = l1_lambda, l2_lambda, dropout
    ...
    def forward(self, data, compute_q=False):
        ...
        return {'x_sim': x_sim, 'x_dist': x_dist, 'fused_rna': fused_rna, 'x_aux': x_aux,
                'fused_joint': fused_joint, 'embedding': fused_joint, 'q_list': None}

    def compute_loss(self, data, outputs, stage=1):
        ...
        l_reg = parameter_regularization_loss(self, self.l1_lambda, self.l2_lambda)
        total_loss = self.beta * total_recon + self.gamma * l_spatial + self.delta * l_reg
        return total_loss, {'loss_total': total_loss.item(), 'loss_recon': total_recon.item(), 'loss_spatial': l_spatial.item()}


class AriseWrapper(nn.Module):
    def __init__(self, in_rna_dim, in_aux_dim, hidden_dim, out_dim, num_clusters,
                 beta=25.0, gamma=10.0, delta=1.0, dropout=0.0):
        super().__init__()
        self.gcn = ARISEBaselineModel(
            in_rna_dim=in_rna_dim, in_aux_dim=in_aux_dim, hidden_dim=hidden_dim, 
            out_dim=out_dim, beta=beta, gamma=gamma, delta=delta, dropout=dropout
        )
        self.cluster_layer = nn.Parameter(torch.Tensor(num_clusters, out_dim))
        nn.init.xavier_uniform_(self.cluster_layer.data)

    def forward(self, data):
        return self.gcn(data)

    def compute_losses(self, data, outputs):
        total_loss, metrics = self.gcn.compute_loss(data, outputs)
        return total_loss, metrics['loss_recon']
...
def train_model(model, data, epochs=350, lr=1e-3, num_clusters=10, true_labels=None, verbose=True):
    ...
    for epoch in pbar:
        ...
        loss, l_rec = model.compute_losses(data, outputs)
        l_rec = torch.tensor(l_rec)
...
def run_experiment(
    ...
    for dataset_name in dataset_names:
        ...
        data = build_multimodal_graphs(RNA_expr, ADT_expr, cell_positions, device=dev)
        ...
        df_ds.to_csv(os.path.join(output_dir, f"AriseBaseV0_{dataset_name}_results.csv"), index=False)

    # Save overall summary
    df_all = pd.DataFrame(all_results)
    df_all.to_csv(os.path.join(output_dir, "AriseBaseV0_all_results.csv"), index=False)
