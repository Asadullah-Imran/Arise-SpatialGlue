...
================================================================================
  ...
  NOTE: This is Base Model + DEC with Kendall & Gal Uncertainty Balancing.
        No Cross-Attention, No Gram Alignment, No Sinkhorn OT.
================================================================================
"""
...
class Dual4Encoder1LayerDEC(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, out_channels, q, num_clusters,
                 delta=1.0, dropout=0.0, l1_lambda=1e-4, l2_lambda=1e-3):
        ...
        # --- Kendall & Gal Homoscedastic Multi-Task Uncertainty Parameters (s_0..s_2) ---
        # Task losses: [L_recon, L_spatial, L_dec]
    ...
    def compute_losses(self, data: Dual4GraphDataDEC, outputs: dict, stage: int = 1):
        ...
        # Stage 2: Add DEC KL loss with its own uncertainty weight
        if stage == 2 and outputs['kl_loss'] is not None:
            l_kl = outputs['kl_loss']
            p2 = torch.exp(-self.log_vars[2])
            total_loss = total_loss + 0.5 * p2 * l_kl + 0.5 * self.log_vars[2]
...
def run_experiment(
    ...
    output_dir: str = "results_kendall_gal_dec",
    data_dir: str = "data"
):
    """
    ...
    print(f"• Loss Weights : Kendall & Gal Uncertainty Balancing (3 Tasks) | Delta={delta}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default='results_kendall_gal_dec', help="Directory to save CSV results and plots")
