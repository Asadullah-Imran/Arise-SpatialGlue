...
def plot_all_visualizations(
    ...
    if spatial_coords is not None:
        ...
        axes[1].set_title(f'ASTRA-Redo Transformer Domains (ARI: {ari:.4f})', fontsize=12, fontweight='bold')
...
def run_experiment(datasets="all", seeds=None, pretrain_epochs=250, finetune_epochs=150, lr=1e-3, 
                   lambda_dec=1.0, lambda_spatial=0.15, output_dir="astra_redo_transformer_Results", 
                   visualize=True):
    ...
    for dataset_name in dataset_names:
        ...
        df_ds.to_csv(os.path.join(output_dir, f"ASTRA_Redo_Transformer_{dataset_name}_results.csv"), index=False)
        
    df_all = pd.DataFrame(all_results)
    df_all.to_csv(os.path.join(output_dir, "ASTRA_Redo_Transformer_all_results.csv"), index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default='astra_redo_transformer_Results', help="Output directory")
