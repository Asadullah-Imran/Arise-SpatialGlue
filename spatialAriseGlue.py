...
# - Fusion: SpatialGlue-style Attention Fusion (Intra-omic + Inter-omic)
...
class AttentionLayer(nn.Module):
    """
    SpatialGlue-style Attention Layer for fusing two embeddings via a learned weighted sum.
    """
...
class DualGCNAll1Layer(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, out_channels, q, dropout=0.0):
        ...
        self.atten_rna = AttentionLayer(out_channels, out_channels)
        self.atten_aux = AttentionLayer(out_channels, out_channels)
        self.atten_cross = AttentionLayer(out_channels, out_channels)
    ...
    def forward(self, x_RNA, x_ADT,
        ...
        # 3. Attention Fusion
        fused_rna, alpha_rna = self.atten_rna(x_sim, x_dist)
        fused_aux, alpha_aux = self.atten_aux(aux_s, aux_d)
        fused_pro, alpha_cross = self.atten_cross(fused_rna, fused_aux)
...
def plot_all_visualizations(
    ...
    rna_pca_comps=60,
    true_labels=None,
    output_dir="results_spatial_arise_glue",
    show=False
):
    """
...
def run_experiment(
    ...
    output_dir: str = "results_spatial_arise_glue",
    data_dir: str = "data"
):
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default='results_spatial_arise_glue', help="Directory to save CSV results and plots")
