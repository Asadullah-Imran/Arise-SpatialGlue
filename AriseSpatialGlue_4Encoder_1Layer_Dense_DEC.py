...
================================================================================
  ...
  NOTE: This is Base Model + Dense Gram Relational Loss + Spatial Potts DEC.
        No Cross-Attention, No Sinkhorn OT, No Kendall & Gal Uncertainty Balancing.
================================================================================
"""
...
class Dual4Encoder1LayerDEC(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, out_channels, q, num_clusters,
                 beta=25.0, gamma=10.0, delta=1.0, kl_weight=1.0, lambda_dense=1.0, dropout=0.0,
    ...
    def compute_losses(self, data: Dual4GraphDataDEC, outputs: dict, stage: int = 1):
        ...
        # Dense Cross-Modal Relational Gram Alignment Loss (V7)
        num_nodes = outputs['fused_rna'].size(0)
        l_emb = torch.mean((outputs['fused_pro'] - outputs['fused_rna']) ** 2) + torch.mean((outputs['fused_pro'] - outputs['fused_aux']) ** 2)
...
def run_experiment(
    ...
    output_dir: str = "results_dense_dec",
    data_dir: str = "data"
):
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_dir', type=str, default='results_dense_dec', help="Directory to save CSV results and plots")
