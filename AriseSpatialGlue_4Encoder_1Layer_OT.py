...
# ==============================================================================
# some number will add. Entropic Sinkhorn Optimal Transport Relational Loss (V12)
# ==============================================================================
...
class SinkhornOptimalTransportLoss(nn.Module):
    ...
    def forward(self, z_rna: torch.Tensor, z_aux: torch.Tensor) -> torch.Tensor:
        ...
        mu = torch.full((N_rna,), 1.0 / N_rna, device=z_rna.device, dtype=z_rna.dtype)
        nu = torch.full((N_aux,), 1.0 / N_aux, device=z_aux.device, dtype=z_aux.dtype)

        K = torch.exp(-C / self.eps)
        u = torch.ones(N_rna, device=z_rna.device, dtype=z_rna.dtype)
...
class Dual4Encoder1Layer(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, out_channels, q, num_clusters,
                 beta=25.0, gamma=10.0, delta=1.0, ot_lambda=1.0,
                ot_eps=0.1,
                ot_max_iter=30,dropout=0.0,
        ...
        self.ot_loss = SinkhornOptimalTransportLoss(
            eps=ot_eps,
    ...
    def compute_losses(self, x_RNA, x_ADT, x_sim, x_dist, aux_s, aux_d,
        ...
        l_ot = self.ot_loss(fused_rna, fused_aux)
        ...
        total_loss = (self.beta * (l_rec + l_sim + l_dist + l_aux_s + l_aux_d) +
                      ...
                      self.delta * reg_loss  + self.ot_lambda * l_ot)

        return total_loss, l_rec, l_ot
...
def train_model(model, data, epochs=350, lr=1e-3, num_clusters=10, true_labels=None, verbose=True):
    ...
    for epoch in pbar:
        ...
        loss, l_rec, l_ot = model.compute_losses(
            data.x_RNA, data.x_ADT,
        ...
        if verbose and ((epoch + 1) % 10 == 0 or epoch == 0):
            ...
            tqdm.write(f"Epoch {epoch + 1:4d}/{epochs} | Total Loss: {loss.item():.4f} | Recon Loss: {l_rec.item():.4f} | OT Loss: {l_ot.item():.4f} | Silhouette: {sil:.4f}{ari_str} | Best Sil: {best_sil:.4f} (Epoch {best_epoch})")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ot_eps',type=float,default=0.1,help="Sinkhorn entropy regularization")
