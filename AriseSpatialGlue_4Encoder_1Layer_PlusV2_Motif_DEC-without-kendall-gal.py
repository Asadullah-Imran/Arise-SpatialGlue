...
"""
================================================================================
  🧬 AriseSpatialGlue_4Encoder_1Layer_PlusV2_Motif_DEC.py

  Complete Grand Champion Architecture:
  4-Encoder 1-Layer Arise with Multi-Order Motif Topology, Synergistic Losses & Spatial Potts DEC:
  1. 📐 4-Stream 1-Layer GCNs (RNA Spatial, RNA Sim with Motifs, Aux Spatial, Aux Sim with Motifs)
  2. 🧬 RNA Dimensionality Reduction via PCA (n_comps = 60 / 100)
  3. 🕸️ Multi-Order Motif Topology (V9): 3-Node Triangular Cliques (M3) + 4-Node Cycles (M4)
  4. ⚡ Spatially-Constrained Cross-Attention Feature Exchange (Graph-Masked)
  5. 🔗 Dense Relational Gram Matrix Alignment (Frobenius Norm)
  6. 🌊 Entropic Sinkhorn Optimal Transport Relational Loss
  7. 🎯 Spatial Potts MRF-Regularized Consensus DEC (Two-Stage Fine-Tuning)
  8. ⚖️ Kendall & Gal Uncertainty Multi-Task Adaptive Loss Weighting (5 Objectives)
  9. 📈 Complete Diagnostic Visual Analytics Suite:
...
# ==============================================================================
# 4. Multi-Order Motif Topology (V9) & 4-Encoder Graph Construction
# ==============================================================================
...
class Dual4GraphMotifPlusData:
    """PyG Data container holding 4 graphs with motif weights, dense spatial adjacency, and spatial mask."""
    def __init__(
        self,
        x_RNA: torch.Tensor,
        x_ADT: torch.Tensor,
...
def build_4encoder_motif_plus_graphs(
    x_rna: np.ndarray,
    x_aux: np.ndarray,
    cell_positions: np.ndarray,
    device: str = 'cpu',
    num_neighbors: int = 15
) -> Dual4GraphMotifPlusData:
    """
    Constructs 4 distinct graph streams enriched with Multi-Order Motifs (V9) and spatial masks:
      1. RNA Similarity Graph with M3 & M4 Motifs (Cosine kNN + Motifs)
      2. RNA Spatial Graph (Euclidean kNN)
      3. Aux Similarity Graph with M3 & M4 Motifs (Cosine kNN + Motifs)
      4. Aux Spatial Graph (Euclidean kNN)
      5. Dense Spatial Adjacency Matrix & Attention Mask
    """
    num_nodes = x_rna.shape[0]

    # 1. RNA Similarity Graph with Multi-Order Motifs (V9)
    ...
    motif_sparse_rna = compute_multi_order_motif_matrix(sim_sparse_rna, k1=1.0, k2=0.5, k3=0.25)
    sim_nz_rna = motif_sparse_rna.nonzero()
    sim_edge_index_rna = torch.tensor(np.array(sim_nz_rna), dtype=torch.long).to(device)
    sim_edge_weight_rna = torch.tensor(np.array(motif_sparse_rna[sim_nz_rna]).flatten(), dtype=torch.float).to(device)

    # 2. Aux Similarity Graph with Multi-Order Motifs (V9)
    ...
    motif_sparse_aux = compute_multi_order_motif_matrix(sim_sparse_aux, k1=1.0, k2=0.5, k3=0.25)
    sim_nz_aux = motif_sparse_aux.nonzero()
    sim_edge_index_aux = torch.tensor(np.array(sim_nz_aux), dtype=torch.long).to(device)
    sim_edge_weight_aux = torch.tensor(np.array(motif_sparse_aux[sim_nz_aux]).flatten(), dtype=torch.float).to(device)
    ...
    return Dual4GraphMotifPlusData(
        x_RNA=x_RNA_tensor,
        x_ADT=x_ADT_tensor,
...
# ==============================================================================
# 8. Model Architecture: 4-Encoder 1-Layer GCN + Motifs + Synergistic Loss + DEC
# ==============================================================================

class Dual4Encoder1LayerMotifPlusV2DEC(nn.Module):
    """
    🧬 4-Encoder 1-Layer Grand Champion Architecture:
      - 4 Streams of 1-Layer GCNs (RNA Spatial, RNA Sim with Motifs, Aux Spatial, Aux Sim with Motifs)
      - Multi-Order Motif Topology (V9: M3 Triangular + M4 4-Node Cliques)
    ...
    def forward(self, data: Dual4GraphMotifPlusData, compute_q: bool = False) -> Dict[str, torch.Tensor]:
        # 1. RNA Stream (1-layer GCNs with Motif-weighted Similarity Graph)
        ...
        # 2. Aux Stream (1-layer GCNs with Motif-weighted Similarity Graph)
    ...
    def compute_loss(self, data: Dual4GraphMotifPlusData, outputs: Dict[str, torch.Tensor], stage: int = 1) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        # 6. Spatial Potts Consensus DEC KL Loss (V14, Stage 2)
...
def train_model_plus_v2_motif_dec(
    model: Dual4Encoder1LayerMotifPlusV2DEC,
    data: Dual4GraphMotifPlusData,
    ...
    Two-Stage Training Loop:
      - Stage 1: Representation Learning (Multi-Task Kendall & Gal Balancing + Motifs)
    ...
    # --------------------------------------------------------------------------
    if verbose:
        print(f"\n[Stage 1/2] Representation Pre-training with Multi-Order Motifs ({pretrain_epochs} Epochs)...")
...
def plot_training_curves(
    ...
    axes[0].set_title(f"Multi-Task Loss Curves (with Motifs)\n{dataset_name}", fontsize=11, fontweight='bold')
...
def plot_spatial_domains(
    ...
    axes[1].set_title(f'Arise 4-Encoder 1-Layer + Motifs + PlusV2 + DEC (ARI: {ari:.4f})', fontsize=12, fontweight='bold')
    axes[1].axis('off')
...
def visualize_all_results(
    ...
    rna_pca_comps: int,
    seed: int,
    plots_dir: str = "results_arise_plus_v2_motif_dec/plots",
    show: bool = False
):
...
def run_experiment(
    ...
    output_dir: str = "results_plus_v2_motif_dec_static",
    beta: float = 25.0,
    gamma: float = 10.0,
    lambda_dense: float = 1.0,
    lambda_ot: float = 1.0,
    ...
    print("\n" + "=" * 80)
    print(" 🚀 STARTING 4-ENCODER 1-LAYER PLUS-V2 + MOTIFS + SPATIAL POTTS DEC BENCHMARK")
    print("=" * 80)
    ...
    print(f"• Graph Encoders: 4-Stream 1-Layer GCN + Multi-Order Motif Cliques (M3/M4, V9)")
    print(f"• Loss Protocol: Multi-Task Recon + Spatial Contrastive + Gram Alignment + Sinkhorn OT + Potts DEC")
    print(f"• Loss Weights : STATIC (β={beta}, γ={gamma}, λ_dense={lambda_dense}, λ_ot={lambda_ot}, κ={kl_weight}, δ={delta})")
    print(f"• Dimensions   : Hidden={hidden_dim}, Out={out_dim} | Device: {device}")
    print("=" * 80 + "\n")
    ...
    for ds_name in target_datasets:
        ...
        for seed in seeds:
            ...
            # 3. Construct 4-Encoder Graphs with Motif Enrichment & Spatial Mask
            data = build_4encoder_motif_plus_graphs(
                x_rna=x_rna_pca,
                x_aux=x_aux_feat,
            ...
            results = train_model_plus_v2_motif_dec(
                model=model,
                data=data,
                pretrain_epochs=pretrain_epochs,
                finetune_epochs=finetune_epochs,
        ...
        ds_csv_path = os.path.join(output_dir, f"Arise4Encoder1Layer_PlusV2_Motif_DEC_{ds_name}.csv")
    ...
    # Save to disk
    full_csv = os.path.join(output_dir, "arise_4encoder_1layer_plus_v2_motif_dec_results.csv")
    agg_csv = os.path.join(output_dir, "arise_4encoder_1layer_plus_v2_motif_dec_aggregated.csv")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Arise 4-Encoder 1-Layer PlusV2 with Motifs & Spatial Potts DEC Runner")
    parser.add_argument(
        '--datasets',
        nargs='+',
        default=['all'],
    ...
    parser.add_argument('--output_dir', type=str, default="results_plus_v2_motif_dec_static", help="Output directory (default: results_plus_v2_motif_dec_static)")
    ...
    parser.add_argument('--gamma', type=float, default=10.0, help="Spatial contrastive loss weight")
    parser.add_argument('--lambda_dense', type=float, default=1.0, help="Dense Gram alignment loss weight")
    parser.add_argument('--lambda_ot', type=float, default=1.0, help="Sinkhorn OT loss weight")
    parser.add_argument('--kl_weight', type=float, default=1.0, help="Spatial Potts DEC KL loss weight")
    parser.add_argument('--delta', type=float, default=1.0, help="L1/L2 Regularization weight")
